...
class LBAD(nn.Module):
    def __init__(self, neurons, activation, drop_out_p):
        ...
        self.bn1 = nn.BatchNorm1d(self.neurons, track_running_stats=False)
...
class Encoder2D(nn.Module):
    def __init__(self, latent_dim, num_joints=17, activation=nn.ReLU):
    ...
    def forward(self, x):
        x = x.reshape(-1, 2*self.num_joints).float()  # x.view(-1, 2*self.num_joints)
...
class Decoder3D(nn.Module):
    ...
    def __build_model(self):
        ...
        self.dec_out_block = nn.Sequential(nn.Linear(self.neurons, 3*self.num_joints),)
