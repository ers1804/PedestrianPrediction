...
class PED_pre_encode_attention(nn.Module):
    def __init__(self, enc_dim, device, num_heads=4, hidden_dim=None, use_lane_info=False, implicit=False):
        super(PED_pre_encode_attention, self).__init__()
        self.device = device
        if hidden_dim is None:
            self.query = simplelinear(input_dim=4, output_dim=enc_dim, device=device)
            self.key = simplelinear(input_dim=4, output_dim=enc_dim, device=device)
            self.value = simplelinear(input_dim=4, output_dim=enc_dim, device=device)
        else:
            self.query = simplelinear(input_dim=4, output_dim=enc_dim, hidden_dim=hidden_dim, device=device)
            self.key = simplelinear(input_dim=4, output_dim=enc_dim, hidden_dim=hidden_dim, device=device)
            self.value = simplelinear(input_dim=4, output_dim=enc_dim, hidden_dim=hidden_dim, device=device)
